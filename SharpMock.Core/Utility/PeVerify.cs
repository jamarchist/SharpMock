...
namespace SharpMock.Core.Utility
{
    ...
    {
        public delegate string ErrorNormalizer(string error);
        ...
        static void ParseErrors(PeVerifyResult result, string stdOut)
        {
            ...
            result.MetaDataErrors = new List<string>();

            ParseILErrors(result, stdOut);
            ParseMetaDataErrors(result, stdOut);
        }

        private static void ParseILErrors(PeVerifyResult result, string stdOut)
        {
            ParseAnyErrors(result.Errors, stdOut, "[IL]:", new List<string>{ "[IL]:", "[MD]:" }, result.NormalizeErrorString);
        }

        private static void ParseMetaDataErrors(PeVerifyResult result, string stdOut)
        {
            ParseAnyErrors(result.Errors, stdOut, "[MD]:", new List<string> { "[IL]:", "[MD]:" }, result.NormalizeErrorString);
        }

        private static void ParseAnyErrors(List<string> errorList, string stdOut, string errorMarker, List<string> allMarkers, ErrorNormalizer normalize)
        {
            int startIndex = 0;
            while (startIndex < stdOut.Length)
            {
                startIndex = stdOut.IndexOf(errorMarker, startIndex);
                if (startIndex == -1) break;

                int endIndex = stdOut.IndexOf(errorMarker, startIndex + 1);
                foreach (var marker in allMarkers)
                {
                    var potentialEndIndex = stdOut.IndexOf(marker, startIndex + 1);
                    if (potentialEndIndex > -1 && potentialEndIndex < endIndex) endIndex = potentialEndIndex;
                }
                ...
                errorList.Add(normalize(stdOut.Substring(startIndex, endIndex - startIndex)));
                startIndex = endIndex;
            }        
        }
